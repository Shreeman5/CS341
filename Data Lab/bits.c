...
int absVal(int x)
{
  ...
  int mirror = signBits + x;

  int oneZero = signBits ^ mirror;
...
int allEvenBits(int x) {
  int number = (0x55 << 8) | 0x55;
  int number2 = (number << 16) | number;
  return ! ((x & number2) ^ number2);
}
/* 
...
int anyEvenBit(int x) {
  int temp = (0x55 << 8) | 0x55;
  return !(!(x & (temp << 16 | temp)));
}
/* 
...
unsigned floatAbsVal(unsigned uf) {
  ...
  unsigned int exponentBit = 0xff & (uf >> 23);
  
  if(exponentBit == 0xff && (uf << 9) != 0x00)
  {
    return uf;
  }
  return ~(1<<31) & uf;
...
unsigned floatNegate(unsigned uf) {
  ...
  unsigned int fractionBit = uf << 9;
  
  if(exponentBit == 0xff && fractionBit != 0x00)
  {
    return uf;
  }
 ...
 */
int isNonZero(int x) {
  int leadingBit = (~x + 1) | x;
  int shiftVal = (leadingBit >> 31) & 1;
  return shiftVal;
}
/* 
...
int logicalShift(int x, int n) {
  int maskingValue = ~(((1 << 31) >> n) << 1);
  return maskingValue & (x >> n);
}
/* 
...
int replaceByte(int x, int n, int c) {
  int mask = 0xFF << (n << 3);
  int bytePosition = c << (n << 3);

  return (x & ~mask) | bytePosition;
